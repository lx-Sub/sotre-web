<template>
  <header>
    ...
    >
      <el-form ref="form" label-width="100px" :model="infoForm.formData"  >
    ...
    <el-dialog title="请假" :visible.sync="leaveForm.isShow">
      <el-tabs type="border-card" value="leave" @tab-click="clickTab">
        <el-tab-pane label="请假" name="leave">
          <el-form ref="leaveForm" label-width="100px"   :model="leaveForm.formData"
                   :rules="leaveForm.rules">
            <el-form-item label="假期类型" prop="typeNum">
              <el-select v-model="leaveForm.formData.typeNum" style="width:38%" :no-data-text="'您当前所属部门没有可请假类型'">
      ...
      textAlign:'center',fontWeight:'bold',borderWidth:'3px'}">
              ...
              <el-table-column label="操作" width="120px" fixed="right">
                <template slot-scope="scope">
                  <el-button v-if="scope.row.staffLeave.status===scope.row.unaudited"   type="primary"
                             @click="cancel(scope.row)"
                  > 撤销</el-button>
                  <el-button v-if="scope.row.staffLeave.status!==scope.row.unaudited"   type="danger"
