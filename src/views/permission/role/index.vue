      ...
      <el-form ref="form" label-width="100px" :model="dialogForm.formData">
        <el-form-item label-width="40px" style="margin-bottom:4px ">
          ...
          <el-form-item label="职称" style="display:inline-block" prop="name">
            <el-input placeholder="请输入职称" v-model.trim="dialogForm.formData.name" />
          </el-form-item>
        </el-form-item>
    ...
    <el-dialog title="菜单分配" :visible.sync="menuDialog.isShow">
      <el-tree ref="tree" :props="menuDialog.props" :data="menuDialog.menuData" node-key="id" show-checkbox
        ...
        <span class="custom-tree-node" slot-scope="{ node, data }">
          <span><i :class="'iconfont icon-r-' + data.icon" /> {{ data.name }}</span>
        </span>
      </el-tree>
      <div slot="footer" class="dialog-footer">
    ...
    <div class="manage-header">
      <el-form label-width="auto" :model="searchForm.formData" :inline="true">
        <el-form-item label="职称" prop="name">
          <el-input placeholder="请输入职称" v-model.trim="searchForm.formData.name" prefix-icon="el-icon-search" />
        </el-form-item>
      </el-form>
    </div>
    <!-------------------------- 数据表格 ------------------>
      ...
      textAlign: 'center', fontWeight: 'bold', borderWidth: '3px'
        ...
        <el-table-column label="操作" width="420" fixed="right" align="center">
          <template slot-scope="scope">
            ...
            <el-button type="warning" @click="selectMenu(scope.row.id)"> 分配菜单</el-button>
          </template>
        </el-table-column>
      </el-table>
